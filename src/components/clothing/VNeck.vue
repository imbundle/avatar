...
import { type ClothingProps } from "./types";
import { injectStrict } from "../../utils/injectStrict";
import Graphic from "../clothinggraphic/Graphic.vue";

const theme = injectStrict(ThemeKey);
const { colors, skin } = theme;

const props = withDefaults(defineProps<ClothingProps>(), {
	isFront: true,
});
...
<template>
	<template v-if="!isFront">
		...
		<path
			...
			stroke-width="12px"
		/>
		<g transform="translate(0 30)" v-if="props.graphic">
			<Graphic :kind="props.graphic" />
		</g>
	</template>
</template>
