...
import { type FacialHairKind } from "./facialHair/types";
import FacialHair from "./facialHair/FacialHair.vue";
...
provide(ThemeKey, { colors, skin: colors.skin[props.skinTone] });

const MapHairHatScale: { [key in HairKind]?: number } = {
	Afro: 0,
	Balding: 0,
	Bobcut: 1.12,
	Long: 1.12,
...
const getScale = computed(() =>
	MapHairHatScale[props.hair] !== undefined ? MapHairHatScale[props.hair] : 1
);

console.log(getScale);

const { base: skinBase, shadow: skinShadow } = colors.skin[props.skinTone];
</script>
<template>
